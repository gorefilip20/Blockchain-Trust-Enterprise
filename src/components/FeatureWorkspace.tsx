...
type Props = { area: WorkspaceArea; onNotify: (message: string) => void; onOpenOrder: () => void };
...
export default function FeatureWorkspace({ area, onNotify, onOpenOrder }: Props) {
  return <div className="feature-workspace"><div className="feature-subnav"><button className="active">Workspace</button><button onClick={() => onNotify('Saved views opened.')}>Saved views</button><button onClick={() => onNotify('Entitlements opened.')}>Entitlements</button><span /><div className="feature-live"><span className="status-dot" />Demo data mode</div></div>{area === 'Portfolio' && <PortfolioArea onNotify={onNotify} />}{area === 'Markets' && <MarketsArea onNotify={onNotify} />}{area === 'Trade' && <TradeArea onOpenOrder={onOpenOrder} onNotify={onNotify} />}{area === 'Research' && <ResearchArea onNotify={onNotify} />}{(['Balances', 'Reports', 'Security center', 'Settings', 'Help center'] as AccountAreaKey[]).includes(area as AccountAreaKey) && <AccountArea area={area as AccountAreaKey} onNotify={onNotify} />}</div>;
}
