...
type Mentor = { id: string; name: string; specialty: string; bio: string; experience_years: number; markets: string; total_students: number; rating: number; youtube_channel?: string; youtube_video_id?: string | null };
...
const strategyVideoIds: Record<string, string> = {
  ...
  '@TrencherMatt': '3YRJ4Jblzvg', 'TrencherMatt': '3YRJ4Jblzvg', '@OrangieWEB3': 'dVxtJGybGfI', '@CryptoGorilla': '6HrZ_uQ-sBg', '@itsvladify': '85qG_F9X0w'
};
...
export default function HomePage() {
  ...
  return (
    <main className="catalog-page">
      ...
      <section id="strategies" className="catalog-section"><div className="section-heading"><div><p className="catalog-kicker">EXPLORE THE DESK</p><h2>Find a strategy that <em>fits your thinking.</em></h2></div><p>Every strategist has a different lens. Browse the collection, open a playbook, and see what resonates.</p></div><div className="catalog-toolbar"><div className="category-pills">{categories.map((category) => <button key={category} className={filter === category ? 'active' : ''} onClick={() => setFilter(category)}>{category}</button>)}</div><label className="catalog-search"><Search size={17} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search strategies or mentors" /></label></div><div className="strategy-catalog">{filtered.map((strategy, index) => <article className="strategy-tile" key={strategy.id} onClick={() => setSelected(strategy)}>{strategyVideoIds[strategy.trader_name] ? <div className="homepage-strategy-video"><iframe src={`https://www.youtube-nocookie.com/embed/${strategyVideoIds[strategy.trader_name]}?rel=0&modestbranding=1`} title={`${strategy.trader_name} ${strategy.title} video`} loading="eager" allow="accelerometer; autoplay; encrypted-media; picture-in-picture" allowFullScreen /><span className="homepage-video-label"><Play size={13} fill="currentColor" /> Watch here</span></div> : <div className={`strategy-cover ${coverColors[index % coverColors.length]}`}><span className="cover-label">{strategy.category}</span><span className="cover-avatar">{initials(strategy.trader_name)}</span><div className="cover-wave" /><strong>{strategy.title.split(' ').slice(0, 3).join(' ')}<br /><i>PLAYBOOK</i></strong></div>}<div className="strategy-tile-body"><p className="tile-meta">{strategy.markets} <span>·</span> {strategy.difficulty}</p><h3>{strategy.title}</h3><p className="tile-author">with <strong>{strategy.trader_name}</strong></p><p className="tile-desc">{strategy.description}</p><button>Explore strategy <ArrowRight size={14} /></button></div></article>)}</div>{filtered.length === 0 && <div className="empty-catalog">No strategies match that search yet. Try another market or mentor.</div>}</section>

      <section id="mentors" className="frontpage-mentor-videos"><div className="section-heading"><div><p className="catalog-kicker">THE PEOPLE BEHIND THE PLAYBOOKS</p><h2>Watch the mentors <em>directly.</em></h2></div><p>Play each mentor lesson here without leaving Blockchain Trust Enterprise.</p></div><div className="frontpage-mentor-video-grid">{featuredMentorVideos.map((featured) => <article className="frontpage-mentor-video-card" key={featured.name}><div className="frontpage-mentor-iframe"><iframe src={`https://www.youtube-nocookie.com/embed/${featured.videoId}?rel=0&modestbranding=1`} title={`${featured.name} mentor video`} loading="eager" allow="accelerometer; autoplay; encrypted-media; picture-in-picture" allowFullScreen /></div><div className="frontpage-mentor-video-copy"><p className="catalog-kicker">MEMECOIN MENTOR</p><h3>{featured.name}</h3><p>{featured.specialty}</p></div></article>)}</div></section>
